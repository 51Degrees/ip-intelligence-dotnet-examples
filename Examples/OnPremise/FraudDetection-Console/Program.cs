...
namespace FiftyOne.IpIntelligence.Examples.OnPremise.FraudDetection;

/// <summary>
/// @example OnPremise/FraudDetection-Console/Program.cs
/// 
/// This example shows how to combine 51Degrees On-premise IP Intelligence
/// properties to determine whether an IP is likely to be acting fraudulently.
/// 
/// You will learn:
/// 
/// 1. How to get diversity properties from the IP Intelligence engine.
/// 2. How to combine diversity values to assess the likelihood of a
/// potentially fraudulent IP address.
/// 
/// This example is available in full on [GitHub](https://github.com/51Degrees/ip-intelligence-dotnet-examples/blob/master/Examples/OnPremise/FraudDetection-Console/Program.cs). 
/// 
...
public class Program
{
    public class Example : ExampleBase
    {
            ...
            TextWriter output)
        {
            ...
            // In this example we use the HardwareDiversity and BrowserDiversity
            // properties
            var hardwareDiversity = ipData.HardwareDiversity.HasValue ?
                ipData.HardwareDiversity.Value : 0;
            var browserDiversity = ipData.BrowserDiversity.HasValue ?
                ipData.BrowserDiversity.Value : 0;
            // If the browser versions are significantly more diverse than the
            // hardware, this may indicate that some devices are using multiple
            // browsers, which can be a sign of fraudulent activity.
            // Many other properties can be used to draw conclusions about the
            // likelihood of fraud, and this is a very basic example that should
            // not be used in production without further testing and tuning.
            var isFraudulent = browserDiversity - hardwareDiversity
                > 2;
            ...
            message.AppendLine($"\tIsFraudulent: {isFraudulent}");
